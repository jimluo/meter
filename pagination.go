...
func setPaginationHeader(c echo.Context, isLast bool) {
	...
	baseURL := GetLocalhostIP()
	link := linkheader.Links{
		{URL: baseURL + "?page=" + strconv.Itoa(page) + "&per_page=" + strconv.Itoa(pageSize), Rel: "self"},
	}
	if !isLast {
		link = append(link, linkheader.Link{URL: baseURL + "?page=" + strconv.Itoa(page+1) + "&per_page=" + strconv.Itoa(pageSize), Rel: "next"})
	}
